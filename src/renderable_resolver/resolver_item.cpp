...
std::optional<RenderableInstance> RenderableResolver::resolve_instance(const ItemSnapshot& item) {
    using namespace renderable_resolver;

    const auto tag = SpriteTag{
        SpriteType::Item,
        static_cast<SpriteName>(item.name),
        item.id
    };
    ...
    */
    ...
    {
        std::cerr << "[RenderableResolver] ERROR: Item: " << static_cast<int>(item.name)
    ...
    */
    ...
    {
        std::cerr << "[RenderableResolver] ERROR: Failed to retrieve resource for item "
