...
std::optional<RenderableInstance> RenderableResolver::resolve_instance(const EnemySnapshot& enemy) {
    using namespace renderable_resolver;

    const auto tag = SpriteTag{
        SpriteType::Enemy,
        static_cast<SpriteName>(enemy.name),
        enemy.id
    };
    ...
    */
    ...
    {
        std::cerr << "[RenderableResolver] ERROR: Enemy: " << static_cast<int>(enemy.name)
    ...
    */
    ...
    {
        std::cerr << "[RenderableResolver] ERROR: Failed to retrieve resource for enemy "
